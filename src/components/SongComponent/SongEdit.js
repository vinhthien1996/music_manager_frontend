...
export default function SongEdit(props) {
    ...
    // GET SONG BY ID
    const getSongById = () => {
        axios.get(`http://localhost:8080/api/song/full/${props.match.params.id}`)
            .then(result => {
                const data = result.data;
                data.release_time = formatDate(data.release_time);
                setState({ ...state, song: data });
                setUpload({...upload, title: data.url});
            })
    ...
    const updateSong = () => {
        if (!state.song.song_name) {
            ...
            axios.put(`${LINK_API}/api/song/${state.song.song_id}`, formData, {
                headers: {
                ...
                .then(result => {
                    ...
                    setState({ ...state, message: "Updated song successfully!", type: "success" });
                })
                .catch(err => {
                    setState({ ...state, message: "", type: "" });
    ...
    return (
        <div className={style.song__container}>
            <div className={style.song__title}>
                <h2>
                    <i className="fa fa-align-justify"></i> Edit Song
                </h2>
                <div className={style.song__title__add}>
                    <Link to={props.location.state ? props.location.state.linkBack : '/'}>
            ...
            <button className={style.btn__song} onClick={() => updateSong()}>Save</button>
